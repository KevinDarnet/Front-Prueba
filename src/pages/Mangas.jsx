...
export default function Mangas() {
  ...
  async function getMangas() {
    ...
    const {
      data: { mangas, count },
    } = await axios.get(`http://localhost:8080/mangas?page=${page}`);
  ...
  function next() {
    if (page < Math.floor(cantidadTotal / 4))
      navigate("/mangas/" + (Number(page) + 1));
  }
  function prev() {
  ...
  return (
    <div className="flex-1 flex flex-col w-full px-8">
      <div
        className="w-full h-[65vh] absolute z-0 top-0 left-0 bg-cover"
        style={{ backgroundImage: `url(/images/portada.svg)` }}
      ></div>
      <div className="w-full h-[50vh] z-10 flex justify-center items-center flex-col gap-16">
        ...
        <div className="flex bg-white w-1/2 p-4 rounded-xl overflow-hidden items-center">
          ...
          <span className="text-2xl cursor-pointer select-none" onClick={prev}>
            ⬅️
          </span>
          pagina {page} de {Math.floor(cantidadTotal / 4) || 1}{" "}
          <span className="text-2xl cursor-pointer select-none" onClick={next}>
            ➡️
          </span>
        </div>
      </div>
      <div className="w-full h-[70vh] z-10 bg-white rounded-t-2xl p-4 flex place-content-center place-items-center gap-8 gap-y-8 flex-wrap">
        {mangas.length < 1 ? (
          ...
          mangas.map((manga) => (
            <Card
              key={manga._id}
              category={manga.category_id}
              cover={manga.cover_photo}
              title={manga.title}
